...
//Анимации
{
    ...
    {
        public Animation(GameScreen game)
        {
            m_game = game;
            m_time = 250;
            m_steps = 10;
    ...
    {
        public SwapAnimation(GameScreen game) : base(game)
        {
            m_steps = 8;
        }
        ...
        {
            Point distance = new Point(b.Location.X - a.Location.X, b.Location.Y - a.Location.Y); //Дистанция между элементами
            Point speed = new Point(distance.X / m_steps, distance.Y / m_steps); //Скорость анимации
            int last = m_steps - 1;
            m_count = 0;

            //Анимация смены цифер в таймере
            m_timer = new Timer();
    ...
    {
        public FallAnimation(GameScreen game) : base(game)
        {
            m_time = 100; //Скорость падения
        }

        //Старт анимации падения
        public void Start(List<VisualElement> elements)
        {
            int distance = elements.First().Size.Height; //Дистанция падения
            int speed = distance / m_steps;
            //сдвиг упавших элементов вверх
            for (int j = 0; j < elements.Count; ++j) //Выщитывание расположения элементов
            {
